...
ReplyInfo : [
    Broadcast(Payload({ type : Str, msg_id : U64, message : U64 }), List([BroadcastOk({ type : Str, msg_id : U64, in_reply_to : U64 }), BroadcastNode(Str, { type : Str, msg_id : U64, message : U64 })]), NodeState),
    Read(Payload({ type : Str, msg_id : U64 }), { type : Str, msg_id : U64, in_reply_to : U64, messages : List(U64) }, NodeState),
    BroadcastNode(Payload({ type : Str, msg_id : U64, message : U64 }), List([BroadcastNodeOk({ type : Str, msg_id : U64, in_reply_to : U64 }), BroadcastNode(Str, { type : Str, msg_id : U64, message : U64 })]), NodeState),
...
handle_input! = |input, loop_state| {
    match loop_state {
        ...
        WaitingForTopology(node_state) => {
            ...
            neighbors =
                match unwrapped_node_state.node_id {
                    "n0" => body.topology.n0
                    "n1" => body.topology.n1
                    "n2" => body.topology.n2
                    "n3" => body.topology.n3
            ...
            reply_info =
                match body_type {
                        ...
                        (messages_to_neighbors, new_neighbors_messages) = Dict.walk(
                            ...
                            |(msgs, n_msgs), k, v| {
                                if Set.contains(v, message) {
                                    ...
                                    (List.append(msgs, BroadcastNode((k, { type: "broadcast_node", msg_id: 0, message: message }))), add_to_dict_set(n_msgs, k, message))
                                }
                            }
                        )
                    ...
                    "broadcast_node" => {
                        ...
                        updated_neighbors_messages = add_to_dict_set(storage.neighbors_messages, p.src, message)
